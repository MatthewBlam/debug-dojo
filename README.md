...
flowchart LR
  ...
  subgraph Frontend["Frontend: Next.js + React"]
    NextApp["App Router"]
    Pages["Routes: login, problems, workspace, submissions, progress"]
    Workspace["Workspace + Monaco Editor"]
    ApiClient["src/lib/api.ts"]
    SupabaseClient["src/lib/supabase.ts + useUser"]
  end
  ...
  subgraph Backend["Backend: FastAPI"]
    ...
    ProblemsAPI["Problems API"]
    RunsAPI["Visible Run API"]
    SubmissionsAPI["Submission API"]
    ProgressAPI["Progress API"]
    JudgePipeline["Judge Pipeline"]
    Complexity["AST Complexity Analyzer"]
  ...
  subgraph Judge0Stack["Judge0 CE Docker Stack"]
    Judge0["Judge0 API"]
    Judge0Worker["Judge0 Worker"]
    Judge0Postgres["Judge0 Postgres"]
    Redis["Redis"]
  end
  ...
  subgraph Tooling["Problem Tooling"]
    Seeds["YAML problem seeds"]
    CLI["seed_problem / slopify"]
  end
  ...
  SupabaseClient <-->|sign in, session, OAuth| SupabaseAuth
  ...
  SubmissionsAPI --> Auth
  ProgressAPI --> Auth
  Auth -. verifies JWT issued by .-> SupabaseAuth

  ProblemsAPI --> SupabaseDB
  SubmissionsAPI -->|insert pending submission| SupabaseDB
  ProgressAPI --> SupabaseDB
  SupabaseDB --> Problems
  SupabaseDB --> Tags
  SupabaseDB --> TestCases
  SupabaseDB --> Submissions
  SupabaseDB --> Profiles

  RunsAPI -->|visible tests only| JudgePipeline
  SubmissionsAPI -->|background full judge| JudgePipeline

  JudgePipeline -->|load problem and tests| SupabaseDB
  JudgePipeline -->|run user and reference wrappers| Judge0
  Judge0 --> Judge0Worker
  Judge0Worker --> Judge0Postgres
  Judge0Worker --> Redis

  JudgePipeline --> Complexity
  JudgePipeline --> Feedback
  Feedback --> Gemini
  JudgePipeline -->|update verdict and feedback| Submissions

  Seeds --> CLI
  CLI -->|service role writes| SupabaseDB
```
